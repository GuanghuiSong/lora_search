...
from models.modeling_roberta_lora import (
    ...
    RobertaLoraForQuestionAnswering,
    RobertaLoraLayer,
)
from models.modeling_roberta_lora_ags import (
    RobertaLoraForCausalLM as RobertaLoraAgsForCausalLM,
    RobertaLoraForSequenceClassification as RobertaLoraAgsForSequenceClassification,
    RobertaLoraForQuestionAnswering as RobertaLoraAgsForQuestionAnswering,
    RobertaLoraAgsLayer
)
...
class DynamicLoraReallocationCallback(pl.Callback):
    ...
    def reallocation(
        ...
        with torch.no_grad():
            ...
            assert (
                type(model) is RobertaLoraForCausalLM
                or type(model) is RobertaLoraForSequenceClassification
                or type(model) is RobertaLoraForQuestionAnswering
                or type(model) is RobertaLoraAgsForCausalLM
                or type(model) is RobertaLoraAgsForSequenceClassification
                or type(model) is RobertaLoraAgsForQuestionAnswering
            )
            model: RobertaLoraForCausalLM | RobertaLoraForSequenceClassification | RobertaLoraForQuestionAnswering | RobertaLoraAgsForCausalLM | RobertaLoraAgsForSequenceClassification | RobertaLoraAgsForQuestionAnswering

            for encoder_layer in reversed(model.roberta.encoder.layer):
                encoder_layer: RobertaLoraLayer
                lora_modules: dict[str, LoraLinear] = {
                    "q_proj": encoder_layer.attention.self.query,
                    "k_proj": encoder_layer.attention.self.key,
                    "v_proj": encoder_layer.attention.self.value,
                    "o_proj": encoder_layer.attention.output.dense,
                    "fc1": encoder_layer.intermediate.dense,
                    "fc2": encoder_layer.output.dense,
                }
                ...
                if isinstance(encoder_layer, RobertaLoraAgsLayer):
                    encoder_layer: RobertaLoraAgsLayer
                    shortcut_modules: dict[str, ShortcutBase] = {
                        "residual_1": encoder_layer.attention.residual_1,
                        "residual_2": encoder_layer.residual_2,
                        "shortcut_sa": encoder_layer.shortcut_sa,
                        "shortcut_ffn": encoder_layer.shortcut_ffn,
                    }
        ...
        with torch.no_grad():
            if self.ags_mode == "combined":
                ...
                model = self.alpha_pl_module.model
                assert (
                    type(model) is RobertaLoraAgsForCausalLM
                    or type(model) is RobertaLoraAgsForSequenceClassification
                    or type(model) is RobertaLoraAgsForQuestionAnswering
                )
                model: RobertaLoraAgsForCausalLM | RobertaLoraAgsForSequenceClassification | RobertaLoraAgsForQuestionAnswering

                for encoder_layer in reversed(model.roberta.encoder.layer):
                    encoder_layer: RobertaLoraAgsLayer
                    layer_id = encoder_layer.layer_id
    ...
    def _grad_norm_test(
        ...
        grads_norm = {}
        for encoder_layer in model.roberta.encoder.layer:
            encoder_layer: RobertaLoraLayer
            layer_id = encoder_layer.layer_id
            lora_modules: dict[str, LoraLinear] = {
                "q_proj": encoder_layer.attention.self.query,
    ...
    def _snip_test(
        ...
        for encoder_layer in reversed(model.roberta.encoder.layer):
            ...
            original_forward[encoder_layer.layer_id] = {}
            for proj_name, lora in lora_modules.items():
                if (
                    lora.active_adapter not in lora.lora_A.keys()
                    or lora.r[lora.active_adapter] == 0
                ...
                original_forward[encoder_layer.layer_id][proj_name] = lora.forward
